...
bool checkCrossed(vi& l1, vi& l2)
{
    bool isCrossed = false;
    if (l1[4] == Direction::VERTICAL && l2[4] == Direction::VERTICAL && l1[0] == l2[0])
    {
        if ((l2[1] <= l1[1]) && (l1[1] <=l2[3]))
            isCrossed = true;
        else if ((l2[1] <= l1[3]) && (l1[3] <= l2[3]))
            isCrossed = true;
    }
    ...
    else if (l1[4] == Direction::HORIZONTAL && l2[4] == Direction::HORIZONTAL && l1[1] == l2[1])
    {
        if ((l2[0] <= l1[0]) && (l1[0] <=l2[2]))
            isCrossed = true;
        else if ((l2[0] <= l1[2]) && (l1[2] <= l2[2]))
            isCrossed = true;
    }
...
int main()
{
    ...
    for (int i = 0; i < K; ++i)
    {
        ...
        buses.push_back(vi{ _x1, _y1, _x2, _y2, _type });
    }
